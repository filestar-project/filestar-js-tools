...
function transactionSerialize(message) {
  const rawCBOR = transactionSerializeRaw(message)
  return Buffer.from(rawCBOR).toString('hex')
}
...
function createPymtChan(from, to, amount, nonce) {
  if (typeof from !== 'string') {
    throw new TypeError('`from` address has to be a string.')
  }
  if (typeof to !== 'string') {
    throw new TypeError('`to` address has to be a string.')
  }
  if (typeof amount !== 'string') {
    throw new TypeError('`amount` address has to be a string.')
  }
  const constructorParams = [to, from]
  const serializedConstructorParams = cbor.serialize(constructorParams)
  ...
  const serializedParams = cbor.serialize(execParams)
  const message = {
    from,
    to: 't01',
    nonce,
...
function settlePymtChan(pch, from, nonce) {
  if (typeof pch !== 'string') {
    throw new TypeError('`pch` address has to be a string.')
  }
  if (typeof from !== 'string') {
    throw new TypeError('`from` address has to be a string.')
  }
...
function updatePymtChan(pch, from, signedVoucherBase64, nonce) {
  ...
  if (typeof signedVoucherBase64 !== 'string') {
    throw new TypeError('`signedVoucher` has to be a base64 string.')
  }
  const cborSignedVoucher = Buffer.from(signedVoucherBase64, 'base64')
  const signedVoucher = cbor.deserialize(cborSignedVoucher)
  const updateChannelStateParams = [
    signedVoucher,
    Buffer.alloc(0),
    Buffer.alloc(0)
  ]
  const serializedParams = cbor.serialize(updateChannelStateParams)
  const message = {
    from,
    to: pch,
    nonce,
...
function signVoucher(unsignedVoucherBase64, privateKey) {
  if (typeof unsignedVoucherBase64 !== 'string') {
    throw new TypeError('`unsignedVoucher` has to be a base64 string.')
  }
  if (typeof privateKey !== 'string') {
    throw new TypeError('`privateKey` has to be a base64 string.')
  }
  const cborUnsignedVoucher = Buffer.from(unsignedVoucherBase64, 'base64')
  ...
  const unsignedVoucher = cbor.deserialize(cborUnsignedVoucher)
...
function createVoucher(
  ...
  const serializedVoucher = cbor.serialize(voucher)
